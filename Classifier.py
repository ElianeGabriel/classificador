...
def carregar_dominios(ficheiro, sheet):
    ...
    try:
        ...
        st.error(f"Ficheiro de domínios não encontrado: **{ficheiro}**. Coloca-o na pasta onde corres a app.")
        st.stop()
...
def run():
    ...
    # Falha cedo se não houver deployment
    ...
    if st.button("🚀 Classificar com LLM", use_container_width=True):
        ...
        if not resultados:
            st.warning("Nada classificado (linhas vazias ou erro na chamada). Verifica o diagnóstico Azure acima.")
        else:
...
# Execução direta (opcional) e integração com app.py
